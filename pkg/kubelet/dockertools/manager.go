...
func (dm *DockerManager) runContainerInPod(pod *api.Pod, container *api.Container, netMode, ipcMode, pidMode string) (kubeletTypes.DockerID, *docker.Container, error) {
	start := time.Now()
	defer func() {
...
func (dm *DockerManager) createPodInfraContainer(pod *api.Pod) (kubeletTypes.DockerID, *docker.Container, error) {
	...
	id, dockerContainer, err := dm.runContainerInPod(pod, container, netNamespace, getIPCMode(pod), getPidMode(pod))
	if err != nil {
		return "", nil, err
	}
...
func (dm *DockerManager) SyncPod(pod *api.Pod, runningPod kubecontainer.Pod, podStatus api.PodStatus, pullSecrets []api.Secret, backOff *util.Backoff) error {
	...
	for idx := range containerChanges.ContainersToStart {
		...
		_, _, err = dm.runContainerInPod(pod, container, namespaceMode, namespaceMode, getPidMode(pod))
